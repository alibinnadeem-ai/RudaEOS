    ...
    <>
      <div className="shell">
        ...
        <header className="hdr">
          ...
          <span className="hdr-badge hb-live">Live</span>
          <span className="hdr-badge hb-sprint">14-Day Sprint</span>
          <span className="hdr-badge hb-ruda">RUDA</span>
          <div className="hdr-right">
            <span className="hdr-clock">{clock}</span>
            {view === 'dashboard' && (
              <button className="hdr-nav-btn" onClick={() => setView('preview')}>Print Preview</button>
            )}
            {view === 'preview' && (
              <button className="hdr-nav-btn primary" onClick={() => setView('dashboard')}>&larr; Dashboard</button>
            )}
          </div>
        </header>
        ...
        <div className={`view ${view === 'dashboard' ? 'active' : ''}`}>
          <div className="kpi-strip">
            <div className="kpi-cell kpi-g"><div className="kpi-lbl">Total Tasks</div><div className="kpi-num">{total}</div><div className="kpi-desc">7 domains</div></div>
            <div className="kpi-cell kpi-a"><div className="kpi-lbl">Progress</div><div className="kpi-num">{prog}%</div><div className="kpi-desc">Sprint completion</div></div>
            <div className="kpi-cell kpi-b"><div className="kpi-lbl">In Progress</div><div className="kpi-num">{ip}</div><div className="kpi-desc">Active now</div></div>
            <div className="kpi-cell kpi-p"><div className="kpi-lbl">Completed</div><div className="kpi-num">{done}</div><div className="kpi-desc">Closed out</div></div>
            <div className="kpi-cell kpi-r"><div className="kpi-lbl">Blocked</div><div className="kpi-num">{bl}</div><div className="kpi-desc">Needs attention</div></div>
          </div>
          ...
          <div className="body">
            ...
            <nav className="sidebar">
              ...
              <div className={`si ${activeCat === 'all' ? 'active' : ''}`} onClick={() => setActiveCat('all')}>
                <span className="si-dot" style={{ background: 'var(--accent2)' }} />
            ...
            <div className="main">
              <div className="toolbar">
                <div className="tb-search">
                  <span className="tb-sicon">&#x2315;</span>
                  <input type="text" placeholder="Search tasks, owners…" value={search} onChange={e => setSearch(e.target.value)} />
                </div>
                <select className="tb-sel" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
                ...
                <button className="tb-add-btn" onClick={() => openModal(null)}>＋ New Task</button>
                <span className="tb-count">{filteredTasks.length} tasks</span>
              </div>
                    ...
                    return (
                      <div className="cat-block" key={cat.id} style={{ animationDelay: `${ci * 0.05}s` }}>
                        <div className="cat-hdr" onClick={() => {
                          ...
                          <div className="cat-n">{catTasks.length} tasks</div>
                          <div className="cat-prog-track"><div className="cat-prog-fill" style={{ width: `${catProg}%`, background: cat.color }} /></div>
                          <div className="cat-pct" style={{ color: cat.color }}>{catProg}%</div>
                          <button className="cat-add" onClick={e => { e.stopPropagation(); openModal(cat.id); }}>＋ Add</button>
                              ...
                              return (
                                <div className="task-card" key={task.id}>
                                    ...
                                    <div className="tc-header">
                                      <input
                                        ...
                                        onBlur={e => {
                                          ...
                                          if (v && v !== task.name) { updateTask(task.id, { name: v }); toast('Task renamed', 'g'); }
                                    ...
                                    <div className="tc-checklist">
                                      {task.checklist.map((item) => (
                                        <div className="cl-item" key={item.id} onClick={() => toggleCheck(task.id, item.id, item.checked)}>
                                          <div className={`cl-cb ${item.checked ? 'on' : ''}`}>
                                  ...
                                  <div className="tc-right">
                                    ...
                                    <MetaRow label="POC"><div className="poc-val">{task.poc || '—'}</div></MetaRow>
                                    <MetaRow label="Due">
                                      <input type="date" className="dl-inp" defaultValue={task.deadline}
                                        onChange={e => { updateTask(task.id, { deadline: e.target.value }); toast('Deadline updated', 'a'); }} />
                                    </MetaRow>
                                    <MetaRow label="Status">
                                      <div className={`st-badge ${SL[task.status]?.c || ''}`} onClick={e => openStMenu(task.id, e)}>
      ...
      <div className={`modal-overlay ${modalOpen ? 'open' : ''}`} onClick={e => { if (e.target === e.currentTarget) closeModal(); }}>
        <div className="modal" onClick={e => e.stopPropagation()}>
          ...
          <div className="form-row">
            ...
            <input className="form-inp" ref={fName} placeholder="e.g. Confirm registry value with RUDA" />
          </div>
          <div className="form-row">
            <label className="form-lbl">Sub-category</label>
            <input className="form-inp" ref={fSub} placeholder="e.g. 1.1 Land Registry" />
          </div>
          <div className="form-row-2">
            <div>
              <label className="form-lbl">Category</label>
              <select className="form-sel" ref={fCat} defaultValue={modalCat || 'land'}>
                {categories.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
              </select>
            </div>
            <div>
              <label className="form-lbl">Status</label>
              <select className="form-sel" ref={fStatus} defaultValue="NS">
                <option value="NS">Not started</option>
                <option value="IP">In progress</option>
                <option value="CP">Completed</option>
                <option value="BL">Blocked</option>
          ...
          <div className="form-row-2">
            <div><label className="form-lbl">Owner</label><input className="form-inp" ref={fOwner} placeholder="Assign to…" /></div>
            <div><label className="form-lbl">POC</label><input className="form-inp" ref={fPoc} placeholder="Point of contact" /></div>
          </div>
          <div className="form-row">
            <label className="form-lbl">Deadline</label>
            <input className="form-inp" type="date" ref={fDeadline} />
          </div>
          <div className="modal-footer">
            <button className="btn-ghost" onClick={closeModal}>Cancel</button>
            <button className="btn-prim" onClick={submitModal}>Create Task</button>
          </div>
        </div>
      </div>

      {/* STATUS MENU */}
      <div className={`st-menu`} style={stMenuTarget && stMenuPos ? { top: stMenuPos.top, left: stMenuPos.left, display: 'flex' } : { display: 'none' }}>
        <div className="sm-o" onClick={() => applyStatus('NS')}><span className="st-dot" style={{ background: 'var(--slate)', width: 7, height: 7, borderRadius: '50%', flexShrink: 0 }} /><span className="sm-o-lbl">Not started</span></div>
        <div className="sm-o" onClick={() => applyStatus('IP')}><span className="st-dot" style={{ background: '#D97706', width: 7, height: 7, borderRadius: '50%', flexShrink: 0 }} /><span className="sm-o-lbl">In progress</span></div>
        <div className="sm-o" onClick={() => applyStatus('CP')}><span className="st-dot" style={{ background: 'var(--accent2)', width: 7, height: 7, borderRadius: '50%', flexShrink: 0 }} /><span className="sm-o-lbl">Completed</span></div>
        <div className="sm-o" onClick={() => applyStatus('BL')}><span className="st-dot" style={{ background: 'var(--red)', width: 7, height: 7, borderRadius: '50%', flexShrink: 0 }} /><span className="sm-o-lbl">Blocked</span></div>
      </div>

      {/* TOASTS */}
      <div className="toast-wrap">
        {toasts.map(t => <div key={t.id} className={`toast t-${t.type} ${t.out ? 'out' : ''}`}>{t.msg}</div>)}
      </div>
    </>
  );
}
...
function MetaRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="tc-meta-row">
      <div className="tc-mk">{label}</div>
      <div className="tc-mv">{children}</div>
    </div>
  );
}

function ChecklistAddRow({ taskId, onAdd }: { taskId: number; onAdd: (taskId: number, text: string) => void }) {
    ...
    <div className="preview-view">
      ...
      <div className="paper">
                ...
                return (
                  <div className="pp-task" key={task.id}>
                    ...
                    <div className="pp-task-right">
                      ...
                      <div className="pp-mr"><span className="pp-mk">Due</span><span className="pp-mv" style={{ fontFamily: 'var(--mono)', fontSize: 11 }}>{task.deadline || '—'}</span></div>
        ...
        <div className="pp-footer">
          <div className="pp-footer-l">RUDA Execution OS — ARD Pvt Ltd — Internal Document</div>
